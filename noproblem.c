 ...
 *                   Music ........................... h0ffman
 *
 * This file contains all the effect code. The demo is generated by this
 * app running on your host computer. All the precalculation and assembling
 * is done at once and a .tap file come out the end ready to go.
 */
...
/* Generate the output .tap file */
void write_tapdata(void)
{
    FILE *f;

    /* Standard Oric tape header */
...
/* This is the text screen wipe at the beginning of the demo */
void gen_scrwipe(uint16_t *addr)
{
    ...
    /* Generate the wipe tab pseudo-randomly. */
...
/******************************************************************************
                           THE STRIPEWOBBLER
 *****************************************************************************/
#define SWOBCOLS  (39)              /* column 0 reserved for colour attribute */
#define SWOBWIDTH (6*39)            /* Width of the stripewobbler in pixels */
#define SWOBMID   (SWOBWIDTH / 2)   /* Midpoint of the stripewobbler */

#define STRIPEWOBBLE_MINSIZE (24)   /* Sine min/max */
#define STRIPEWOBBLE_MAXSIZE (56)
#define STRIPEWOBBLE_MIDDLE (STRIPEWOBBLE_MAXSIZE-STRIPEWOBBLE_MINSIZE)
#define STRIPEWOBBLE_SINTABSIZE (256) /* Size of the sine table */

/* This function generates a function to draw one scanline of the stripes
 * at the given size (pixels), centred.
 */
void gen_stripewobblerline(int size, uint16_t *addr)
{
    ...
    /* Pick the colour for this scanline */
    ...
    /* Find first on/off toggle so that the on/off bit setting is consistent across sizes */
    onoff = 0;
    while (nextswap >= size)
    {
        onoff ^= 1;
    ...
    /* We build the line byte by byte (0x40 is 6 blank pixels on Oric) */
    thisbyte = 0x40;
    lastbyte = 0x80;
    /* Iterate over the bytes for this scanline */
    for (i=0; i<SWOBWIDTH; i++)
    {
        /* Have we reached the point where we need to toggle between stripe and bg? */
        if (i >= nextswap)
        {
            onoff ^= 1;
            nextswap += size;
        ...
        /* Have we filled a bytes worh of pixels? */
        if ((i%6) == 5)
        {
            /* Yes. If it's the same as the previous byte, we just output a
             * store instruction.
             */
            ...
            {
                /* Otherwise write the calculated byte */
                snprintf(code, sizeof(code),
                    "    LDA #$%02x\n"
                    "    STA (ZPTMP),Y\n"
...
void gen_stripewobbler(uint16_t *addr)
{
    ...
    /* This routine takes the scanline to draw as index in X, and
     * puts the corresponding linedraw routine address into ZPTMP
     */
    assemble("stripewobbler_tabxtozptmp:\n"
             ...
             // Main routine exit point 
             ".swfinish:\n"    // Wait for the music to get to the required point
             ...
             "    LDY #24\n"
             ".clrwipetab:\n"
             "    STA ZPWIPETAB,Y\n"    // Can't remember why we clear this?
             "    DEY\n"
             "    BPL .clrwipetab\n"
             "    STA ZPSWFRAME\n"      // SWFRAME / SWFRAME2 are sine table offsets
             "    STA ZPSWFRAME2\n"
             "    STA ZPSWTIME\n"   
             "    STA ZPSWFADE\n"       // If SWFADE = 1, we are in the process of fading out
             "    STA ZPSWSIZE\n"       // SWSIZE is for the reveal at the start (size in lines)
             "stripewobbler_loop:\n"
             "    LDA ZPSWFADE\n"
             "    BEQ .notfading\n"
             "    INC ZPSWTIME\n"
    ...
    snprintf(tmp, sizeof(tmp),
             "    LDX #%u\n"            // If we're fading, we copy the colour set attributes
             "    LDY #3\n"             // downwards towards black.
...
/* This structure describes how to translate the source pixels from the texture
 * to a destination screen byte.
 */
